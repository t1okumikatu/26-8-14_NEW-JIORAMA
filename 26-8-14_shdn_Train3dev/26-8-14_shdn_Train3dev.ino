...
uint8_t targetSpeed3 = 0;
uint32_t currentSpeed3 = 0; 
...
void OnDataRecv(const uint8_t *mac_addr, const uint8_t *incomingData, int len) {
  ...
  if (len >= sizeof(CombinedPayload)) {
    ...
    if (temp.header == 99) {
      ...
      // 🟢 通常の運行データ (data88 == 88)
      if (temp.data88 == 88) {
        targetSpeed3 = temp.train3_speed;
        targetSpeed3 = targetSpeed3*2.5 ; // 速度計算
        Serial.print("targetSpeed3  ");
        Serial.println(targetSpeed3);
        recvData = temp;

        // 🔌 ディープスリープ判定
        /*
        if (targetSpeed3 == 0 && temp.train3_poji == 22) {
          if (justWokeUp == false) {
...
void loop() {
  ...
  if (targetSpeed3 == 0) {
    ...
    if (currentSpeed3 < targetSpeed3) {
      currentSpeed3++; 
    } else if (currentSpeed3 > targetSpeed3) {
      currentSpeed3--; 
    }
    forward(currentSpeed3); 
  }
  //forward(100);//110>>130>150
  //delay(10); 
}